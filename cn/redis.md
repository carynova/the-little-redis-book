...
Redis并没有官方支持Windows平台，但还是有其他选择。你不会想在这里配置实际的生产环境，不过在我过往的开发经历里并没有感到有什么限制。

微软开放技术有限公司（Microsoft Open Technologies, Inc.）提供了一个移植，可以在这里找到：<https://github.com/MSOpenTech/redis>。如文档上描述的，这个移植并不适合生产环境。

另一种解决方式，一般来讲都是有效的，可以在这个页面找到：<https://github.com/dmajkic/redis/downloads>。你可以下载最新的版本（应该会在列表的最上方）。
解压zip文件，然后根据你的系统架构，打开`64bit`或`32bit`文件夹。
...
很快你就会发现，Redis的API就如一组定义明确的函数那般容易理解。Redis具有让人难以置信的简单性，其操作过程也同样如此。这意味着，无论你是使用命令行程序，或是使用你喜欢的语言来驱动，整体的感觉都不会相差多少。因此，相对于命令行程序，如果你更愿意通过一种编程语言去驱动Redis，你不会感觉到有任何适应的问题。如果真想如此，可以到Redis的[客户端推荐页面](http://redis.io/clients)下载适合的Redis驱动。
...
Redis有虚拟存储器的支持。然而，这个功能已经被认为是失败的了（被Redis的开发者），而且它的使用已经被废弃了。
...
现在你可能会想，这很好，但似乎没有什么意义。你不能有效地提取出JSON对象中的一些属性，或者为其附加一些属性。你是对的，这里的经验是，一些命令，尤其是关于字符串数据结构的，只有在给定了明确的数据类型后，才会有实际意义。
...
Redis 2.6包含了一个内建的Lua解释器，开发者可以利用它编写更高级的查询语句，于Redis当中运行。你可以认为这个功能与大多数关系型数据库中的存储过程很相似。

要掌握这个功能，最困难的部分就是学习Lua。幸好，Lua与多数通用机器语言（general purpose language）相似，它文档丰富，拥有一个活跃的社区，并且在Redis之外也有广泛应用。这个章节不会涉及Lua的细节，但希望我们能从几个例子得到简单的引导。

### 为什么？（Why？）

在开始学习使用Lua脚本之前，你可能会想问为什么我们会使用它。许多开发者讨厌传统的存储过程，这和存储过程又有何不同？简短的答案是，它们没什么不同。一旦使用不当，Redis的Lua脚本会对测试代码造成困难，会将业务逻辑与数据存取绑定，甚至重复一些逻辑代码。

但我们适当地使用Lua脚本，这个功能可以简化代码和提升性能。这些好处大部分归功于将多个命令和一些简单的逻辑组织进一个自定的内聚函数（cohesive function）。代码简化了，因为每个Lua脚本的调用不会被打断，这提供了一个简洁的方式来创建我们自己的原子操作（特别是不再需要使用繁琐的`watch`命令）。中间变量将不会产生，所有输出结果会在脚本内部计算完成，从而提升了性能。

之后小节中的例子将会更好地描述这些点。

### Eval

`eval`命令接受一个Lua脚本（一个字符串）、一组将要操作的关键字和一组额外参数，作为方法的参数。让我们一起看一个简单的例子（在Ruby下之行，因为在Redis自身的命令行工具执行多行代码并不是一件趣事）：

	script = <<-eos
	  local friend_names = redis.call('smembers', KEY[1])
	  local friends = {}
	  for i = 1, #friend_names do
	    local friend_key = 'user:' .. friend_names[i]
		local gender = redis.call('hget', friend_key, 'gender')
		if gender = redis.call('hget', friend_key, 'gender')
...
上面的代码获取了Leto所有男性朋友的详细信息。请注意我们在脚本中调用Redis命令的方法是`redis.call("command", ARG1, ARG2, ...)`。

如果你初次接触Lua，你应该仔细地阅读每一行代码。如果能明白这些会更好：`{}`创建了一个空表（表可以作为数组或者字典使用）；`#TABLE`返回了`TABLE`中的元素个数；`..`用于连接两个字符串。

`eval`实际上接受四个参数。第二个参数是有多少个关键字，但是Ruby的驱动自动为我们计算了这个参数。为什么需要关键字个数？考虑我们如何在命令行界面（CLI）执行这个命令：

	eval "....." "friends:leto" "m"
	vs
	eval "....." 1 "friends:leto" "m"

在第一个例子（错误的例子）中，如何让Redis明白那些参数是关键字而哪些是额外参数？在第二个例子中则没有这个歧义。

这里会出现第二个问题：为什么关键字需要明确地列出？所有实际运行的Redis命令都需要关键字。这样做可以允许将来的工具，如Redis Cluster，分发请求到多个Redis服务器。你可能注意到了，我们的例子中实际上动态地读取了关键字（并不是直接将关键字传给`eval`）。An `hget` is issued on all of Leto's male friends. 这是因为预先列出所有关键字这一行为只是建议而并不是规定。上面的代码在单例环境中能够很好地运行，多例也可以，但不包含未发布的Redis Cluster。
